...
#  PJM [2009-10-08] Added handling for the ACL XML
#  Added parser to build a grant and owner object
#  Added XML generation function to construct a full policy for submission back
...
if __name__ == '__main__':
    ...
    if AWS_ACCESS_KEY_ID != 'CHANGEME':
        ...
        if len(entries):
            testpass += 1
            for e in entries:
                print " :: ".join((str(e.name), str(e.creation_date)))
        ...
        ################################
        # Setting the bucket ACL using XML
        ################################
        testcount += 1
